...
- `chat-bridge-app-icon.png`：品牌展示图，1254 × 1254，透明外边距。
- `chat-bridge-logo.png`：透明背景角色，用于独立展示。
- `../../../apps/macos/Resources/AppIcon.icon`：原生 Icon Composer 源文件，沿用透明角色与雾蓝渐变；系统负责圆角与外边距。
- `../../../apps/macos/Resources/Assets.car`、`AppIcon.icns`：原生图标编译产物，包含旧版 macOS 使用的静态尺寸。
...
角色母版使用 Codex 内置图像生成工具完成。桌面图标使用 Icon Composer 的独立背景和角色图层，避免 macOS 给带透明外边距的旧图标再套浅色底板、缩小主体。菜单栏图形保持单色。

修改图标时，用 Xcode 26 或更新版本打开 `AppIcon.icon`，然后在仓库根目录执行：

```sh
bash scripts/build-icon.sh
```

源文件和两个编译产物须一起提交。应用打包直接复制编译产物，ARM 与 Intel 的发布任务均可继续使用 Xcode 16.4，最低系统为 macOS 13.5。
